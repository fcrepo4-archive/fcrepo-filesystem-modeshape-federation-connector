...
public class FileSystemConnector extends WritableConnector implements Pageable {
    ...
    public void initialize( final NamespaceRegistry registry,
        ...
        if (!directory.exists() || !directory.isDirectory()) {
            final String msg = JcrI18n.fileConnectorTopLevelDirectoryMissingOrCannotBeRead.text(getSourceName(), "directoryPath");
        ...
        if (EXTRA_PROPERTIES_JSON.equalsIgnoreCase(extraPropertiesStorage)) {
            final JsonSidecarExtraPropertyStore store = new JsonSidecarExtraPropertyStore(this, translator());
            ...
            final LegacySidecarExtraPropertyStore store = new LegacySidecarExtraPropertyStore(this);
            setExtraPropertiesStore(store);
            filenameFilter.setExtraPropertiesExclusionPattern(store.getExclusionPattern());
    ...
    protected String idFor( final File file ) {
        ...
        if (!path.startsWith(directoryAbsolutePath)) {
            ...
            final String msg = JcrI18n.fileConnectorNodeIdentifierIsNotWithinScopeOfConnector.text(getSourceName(), directoryPath, path);
     ...
     */
    protected ExternalBinaryValue createBinaryValue( final File file ) throws IOException {
     ...
     *         readonly
     */
    protected void checkFileNotExcluded( final String id,
            final File file ) {
        if (isExcluded(file)) {
            final String msg = JcrI18n.fileConnectorCannotStoreFileThatIsExcluded.text(getSourceName(), id, file.getAbsolutePath());
    ...
    public Document getDocumentById( final String id ) {
        ...
        if (isResource) {
            ...
            final String childId = isRoot ? JCR_CONTENT_SUFFIX : id + JCR_CONTENT_SUFFIX;
    ...
    private DocumentWriter newFolderWriter( final String id,
            final File file,
            final int offset ) {
        ...
        for (int i = 0; i < children.length; i++) {
            ...
            if (child.exists() && child.canRead() && (child.isFile() || child.isDirectory())) {
                ...
                if (i >= offset && i < offset + pageSize) {
                    ...
                    final String childId = root ? DELIMITER + childName : id + DELIMITER + childName;
    ...
    public void storeDocument( final Document document ) {
        ...
        if (!parent.canWrite()) {
            ...
            final String msg = JcrI18n.fileConnectorCannotWriteToDirectory.text(getSourceName(), getClass(), parentPath);
            throw new DocumentStoreException(id, msg);
        }
        ...
        try {
            if (NT_FILE.equals(primaryType)) {
                ...
                final BinaryValue binary = factories().getBinaryFactory().create(content.getFirstValue());
    ...
    public String newDocumentId( final String parentId,
            final Name newDocumentName,
            final Name newDocumentPrimaryType ) {
        ...
        final String childNameStr = getContext().getValueFactories().getStringFactory().create(newDocumentName);
    ...
    public void updateDocument( final DocumentChanges documentChanges ) {
        ...
        final DocumentChanges.ChildrenChanges childrenChanges = documentChanges.getChildrenChanges();
        ...
        for (final String renamedChildId : renamedChildren.keySet()) {
            ...
            final String newNameStr = getContext().getValueFactories().getStringFactory().create(newName);
